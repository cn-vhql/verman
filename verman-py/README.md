...
- 本地优先，项目元数据统一放在工作区下的 `.verman/`
- 根目录只保留用户可见的 `.vermanignore`
- 不做云同步，不包含 VIP、激活或商业化模块
- 打开旧项目时会自动兼容根目录旧布局，并迁移到 `.verman/`
- 当前不再对单个文件大小设置硬性上限
...
- `.verman/project.db` 是版本数据库
- `.verman/backup/` 是回滚前自动备份目录
- `.vermanignore` 继续放在根目录，方便直接编辑
